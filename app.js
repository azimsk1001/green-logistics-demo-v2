...
function exportCSV() {
  ...
  lines.push(`Vehicle,${csvEscape(r.vehicle.model)} (${csvEscape(r.vehicle.euro)})`);
  lines.push(`WTT included,${r.inputs.wtt_on ? 'Yes' : 'No'}`);
  lines.push(`Load (kg),${r.inputs.load_kg}`);
  lines.push(`Fuel base (L/100km),${r.inputs.fuel_l_per_100km_base}`);
  lines.push(`NOx base (g/km),${r.inputs.nox_g_per_km_base}`);
  lines.push(`PM base (g/km),${r.inputs.pm_g_per_km_base}`);
  lines.push(`Fuel adj factor,${r.inputs.fuel_factor}`);
  lines.push(`NOx adj factor,${r.inputs.nox_factor}`);
  lines.push(`Total distance (km),${r.route.total_distance_km}`);
  lines.push(`Total fuel (L),${r.totals.fuel_l}`);
  lines.push(`CO2 TTW (kg),${r.totals.co2_ttw_kg}`);
  lines.push(`CO2 TTW+WTT (kg),${r.totals.co2_ttw_wtt_kg}`);
  lines.push(`NOx (kg),${r.totals.nox_kg}`);
  lines.push(`PM (kg),${r.totals.pm_kg}`);
  lines.push('');

  // Route order
  lines.push('Route order');
  r.route.points.forEach((name, i) => {
    lines.push(`${i + 1},${csvEscape(name)}`);
  });
  lines.push('');

  // Per-leg table (if any)
  if (r.route.legs_km && r.route.legs_km.length) {
    lines.push('Leg,From,To,Distance (km)');
    for (let i = 0; i < r.route.legs_km.length; i++) {
      const from = r.route.points[i] || '';
      const to   = r.route.points[i + 1] || '';
      lines.push(`${i + 1},${csvEscape(from)},${csvEscape(to)},${r.route.legs_km[i]}`);
    }
  }
  ...
  const url = URL.createObjectURL(blob);
  triggerDownload(url, makeFileName('route_report', 'csv'));
}
...
function addDestinationRow() {
  ...
  renderSuggestions(suggEl, list, place => { setDestination(destObj, place); inputEl.value = place.fullName; selectedEl.textContent = Selected: ${place.name}; suggEl.style.display = 'none'; updateCounts(); updateCalcEnabled(); });
});
      updateCounts();
      updateCalcEnabled();
    });
...
function renderResults({ distanceKm, legsKm, points, fuelL, co2kg, co2kg_wtt, noxkg, pmkg, fuelFactor, noxFactor, loadKg }) {
  ...
  results.innerHTML = `
    <div class="result-item">
      ...
      <div class="muted">Load: ${Math.round(loadKg)} kg | Fuel adj: ${((fuelFactor - 1) * 100).toFixed(1)}% | NOx adj: ${((noxFactor - 1) * 100).toFixed(1)}%</div>
    </div>
    ${legsHtml ? `<h2>Legs</h2>${legsHtml}` : ''}
  `;

  // Build lastReport for export
  const model = document.getElementById('modelSelect')?.value || '';
  const euro  = document.getElementById('euroSelect')?.value || '';
  const fuelBase = parseFloat(document.getElementById('fuelInput')?.value || '0');
  const noxBase  = parseFloat(document.getElementById('noxInput')?.value || '0');
  const pmBase   = parseFloat(document.getElementById('pmInput')?.value || '0');
  const wttOnVal = !!wttOn;

  lastReport = {
    timestamp: new Date().toISOString(),
    vehicle: { model, euro },
    inputs: {
      fuel_l_per_100km_base: fuelBase,
      nox_g_per_km_base: noxBase,
      pm_g_per_km_base: pmBase,
      wtt_on: wttOnVal,
      load_kg: loadKg,
      fuel_factor: fuelFactor,
      nox_factor: noxFactor
    },
    route: {
